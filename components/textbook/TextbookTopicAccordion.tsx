        ...
        return (
          ...
          >
            <button
              ...
              className="w-full flex items-center gap-2 text-left px-4 py-3 font-semibold text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                  ...
                  <div className="p-4 space-y-6">
                    {sections.map(({ sectionNum, sectionName, topics }) => (
                      <div key={`${topicNum}-${sectionNum}`}>
                        <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">{sectionName}</h3>
                        <ul className="space-y-3">
                          {topics.map((t) => (
                            <li key={t.topic_id}>
                              <Link
                                href={`/textbook/${t.topic_id}`}
                                className="block rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 px-4 py-3 text-left hover:border-primary/30 hover:bg-primary/5 dark:hover:bg-primary/20 transition-colors"
                              >
                                <span className="font-mono text-sm text-primary">{t.topic_id}</span>
                                <span className="ml-2 font-medium text-gray-900 dark:text-gray-100">{t.title}</span>
                                {t.description && (
